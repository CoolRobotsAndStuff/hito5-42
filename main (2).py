...
def AI_TIP(env_vars):
    ...
    if option == '1':
        ...
        ok, tip = apis.get_advice_gemini(w_consult, api_key=env_vars[GEMINI_API_KEY])
        if ok:
            print(f"\n Consejo: {tip}")
        else:
        ...
        ok, tip = apis.get_advice_gemini(weather, api_key=env_vars[GEMINI_API_KEY])
        if ok:
            print(f"\n Consejo: {tip}")
        else:
            print(f" Error al obtener consejo: {tip}")
