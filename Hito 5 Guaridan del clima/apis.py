...
file_created = False  # Asegurate de que esté definido en el módulo

def create_env_file(filename, variables):
    with open(filename, 'w') as f:
        for key, value in variables.items():
            f.write(f"{key}={value}\n")
    return filename  # ahora devuelve el nombre del archivo

def read_env_file(apikeys: dict, filename="apis.env"):
    global file_created
    if not file_created:
        create_env_file(filename, apikeys)
        file_created = True
...
def get_weather(place, openweathermap_api_key: str = None) -> Weather:
    ...
    ok, clima  = get_weather_openweathermap(place, openweathermap_api_key)
    if ok:
        return clima
